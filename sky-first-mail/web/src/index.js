...
async function routeApi(request,env){
  ...
  if(path==='/api/bootstrap'&&request.method==='POST'){
    ...
    try{
      ...
      if(!user?.id) throw new Error('Không thể xác định tài khoản quản trị sau khi tạo.');
      const userId=Number(user.id);

      stage='preferences';
      await env.DB.prepare(`INSERT OR IGNORE INTO user_preferences(user_id) VALUES(?)`).bind(userId).run();

      // Email đăng nhập có thể là Gmail/Outlook. Chỉ tạo mailbox tự động khi domain
      // đã được khai báo là domain nhận thư của Sky First Mail.
      stage='mailbox';
      ...
      if(managed){
        await env.DB.prepare(`INSERT OR IGNORE INTO mailboxes(user_id,address,display_name,is_primary,is_active) VALUES(?,?,?,1,1)`)
      ...
      await markSetup(env);
      stage='welcome';
      await notify(env,userId,'Chào mừng đến Sky First Mail','Tài khoản Super Admin đã được khởi tạo.','welcome');
      await audit(env,userId,'bootstrap.completed','user',userId,{email});
      stage='session';
      const s=await createSession(env,userId,request);
      console.log('BOOTSTRAP_COMPLETED',{userId,email,mailboxCreated:!!managed});
      return json({ok:true},200,{'set-cookie':s.header});
    }catch(e){
...
export default {async fetch(request,env){try{const url=new URL(request.url);if(url.pathname.startsWith('/api/')){await ensureSchema(env);return await routeApi(request,env)}return env.ASSETS.fetch(request)}catch(e){console.error('APP_ERROR',e);return json({ok:false,error:'Sky First Mail chưa thể hoàn tất yêu cầu. Vui lòng thử lại.',detail:String(e?.message||e)},500)}}};
