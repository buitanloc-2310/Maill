...
export async function hashPassword(password, saltB64 = null, iterations = 100000) {
  ...
  const normalizedIterations = Math.max(10000, Number(iterations || 100000));
  try {
    ...
    // Compatibility fallback so first-run setup can still complete on runtimes where PBKDF2 fails unexpectedly.
    const combined = new Uint8Array(salt.length + encoder.encode(password).length);
    combined.set(salt, 0);
    combined.set(encoder.encode(password), salt.length);
    const digest = await crypto.subtle.digest('SHA-256', combined);
    return { salt: bytesToBase64(salt), hash: `sha256$${bytesToBase64(new Uint8Array(digest))}`, iterations: 1 };
  }
}
